...
def write_rlen_clauses_to_file(vec,sequential_vars,vec_extra_vars,file): 
    ...
    if k == 0: 
        ...
        # print(len(vec_extra_vars))
        #assert(len(vec_extra_vars) == 0)
        for i in range(len(vec)):
            for j in range(len(vec[0])): 
                file.write(str(vec[i][j]) + " 0\n")
    else: 
...
def run_one_instance(t_number_of_queries, r_number_of_records, n_domain_size):
    ...
    range_rlen_solver = RangeRlenSolver(
    ...
    time_result, recovery_rate = range_rlen_solver.solve(D_matrix, [sum(x) for x in L])
