...
def gen_S_sets(H_matrices, range_to_min_cover, range_to_min_cover_indices): 
    ...
    for i in range(num_ranges): 
        ...
        for j in range(i, num_ranges): 
            ...
            # if (alpha,beta) in output: 
            #     output[(alpha,beta)][tuple(map(tuple, matrix))].add((i,j))
            # else: 
            #     output[(alpha,beta)] = defaultdict(set)
            #     output[(alpha,beta)][tuple(map(tuple, matrix))].add((i,j))
            ...
            # if i != j: 
            #     if (beta,alpha) in output: 
            #         output[(beta,alpha)][tuple(map(tuple, matrix_t))].add((j,i))
            #     else: 
            #         output[(beta,alpha)] = defaultdict(set)
            #         output[(beta,alpha)][tuple(map(tuple, matrix_t))].add((j,i))
...
def compute_qeq_extra_vars(S, qeq_leakage, t, curr_num): 
    ...
    for i in range(t): 
        for j in range(i+1,t): 
            ...
            output[(i,j)] = [curr_num + 1 + x for x in range(len(S[(a,b)][tuple(A)]))]
            extra_vars += len(S[(a,b)][tuple(A)])
            extra_constraints += 2 * len(S[(a,b)][tuple(A)])+ 1
            curr_num += len(S[(a,b)][tuple(A)])
...
class OSTLeakageSolver:
    ...
    def compute_all_possible_range_matrices(self,qeq_leakage): 
        ...
        S = gen_S_sets(self.list_of_H_matrix,self.range_to_min_cover,self.range_to_min_cover_indices)
        ...
        while candidates and time.time() < t_end: 
            ...
            if (i in visited) and (j in visited): # we've already checked 
                continue
            old_candidate_ranges = candidate_ranges.copy() 
            ...
            for candidate_range in old_candidate_ranges: 
                if candidate_range[i] == -1 and candidate_range[j] == -1: # both are currently unset 
                    for match in matches: 
                        ...
                        tmp[j] = match[1] 
                        to_append = True 
                        pairs = [(x,y) for x in visited for y in [i,j]]
                        for x,y in pairs: 
            ...
            visited.add(i)
            visited.add(j)
            # if len(visited) > 0.75 * self.t_number_of_queries: 
            #     break
            print(len(candidate_ranges))
            print(visited)          
